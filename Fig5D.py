...
for i in range(len(year)):
    ...
    mu = np.mean(all_res)
    n, bins, patches = plt.hist(all_res, num_bins, normed=1, histtype='step', facecolor=color, linewidth=1, alpha=0.3)
    ...
    ndist=ndist/norm/(bbins[1]-bbins[0])
    plt.plot(bbins, ndist, c=color,lw=2)
